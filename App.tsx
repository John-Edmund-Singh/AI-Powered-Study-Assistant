...
import VoiceAssistantIntro from './components/VoiceAssistantIntro';
...
const App: React.FC = () => {
  ...
  const [mindMapConcept, setMindMapConcept] = useState<string>('');
  const [isVoiceAssistantEnabled, setIsVoiceAssistantEnabled] = useState<boolean>(true);
  const [showVoiceIntro, setShowVoiceIntro] = useState<boolean>(false);
  ...
  const handleShowMindMap = (concept: string) => {
    setMindMapConcept(concept);
      ...
      <div className="flex flex-col min-h-screen bg-gray-50 dark:bg-gray-900">
        <Header 
          ...
          onToggleVoiceAssistant={() => {
            const newState = !isVoiceAssistantEnabled;
            setIsVoiceAssistantEnabled(newState);
            if (newState && !localStorage.getItem('voiceIntroSeen')) {
              setShowVoiceIntro(true);
              localStorage.setItem('voiceIntroSeen', 'true');
            }
          }}
        />
          ...
          {selectedPlanId && (
            <div className="mt-8 grid gap-8 md:grid-cols-2">
              <StudyPlanDisplay 
                ...
                studyGoal={history.find(h => h.id === selectedPlanId)?.goal}
                onShowMindMap={handleShowMindMap}
        ...
        {user && (
          <VoiceAssistant
            userId={user.id}
            studyContext={selectedPlanId ? history.find(h => h.id === selectedPlanId)?.goal : undefined}
            onNewMessage={(userMessage, assistantMessage) => {
              // Auto-expand chat and add messages - we'll handle this differently
              console.log('Voice message received:', userMessage.content);
            }}
            isEnabled={isVoiceAssistantEnabled}
          />
        )}

        {/* Chat Box - Only show when user is logged in */}
        {user && (
          <ChatBox
            userId={user.id}
            studyContext={selectedPlanId ? history.find(h => h.id === selectedPlanId)?.goal : undefined}
        ...
        {/* Voice Assistant Intro Modal */}
        <VoiceAssistantIntro 
          isVisible={showVoiceIntro}
          onDismiss={() => setShowVoiceIntro(false)}
        />
      </div>
      </div>
    </div>
